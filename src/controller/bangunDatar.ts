...
/** const atau let itu digunakan
 * untuk mendeklarasikan sebuah variable/
...
const luasPersegiPanjang = (request: Request, response: Response) => {
  try {
    const p = Number(request.body.p);
    const l = Number(request.body.l);
    const luas = p * l;
    return response.status(200).json({
      status: true,
      p,
...
const luasSegitiga = (request: Request, response: Response) => {
  try {
    const a = Number(request.body.a);
    const t = Number(request.body.t);
    const luas = (1 / 2) * a * t;
    return response.status(200).json({
      status: true,
      a,
